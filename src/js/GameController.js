...
export default class GameController {
  ...
  init() {
    ...
    localStorage.removeItem('activeEnemy');
    this.status = 1;
    let indexesList = [];
    ...
    if (this.level === 1) {
      ...
      this.enemyTeam = GameRule.generateEnemyTeam();

      this.playerTeam.characters.forEach(element => {
        this.renderTeam(Pose.playerPosition(this.gamePlay.boardSize), element, indexesList);
      });
    ...
    this.enemyTeam.characters.forEach(element => {
      this.renderTeam(Pose.enemyPosition(this.gamePlay.boardSize), element, indexesList);
    });
  ...
  showOptions(index) {
    if (localStorage.nextPlayer === '0') {
      if (localStorage.activePlayer) {
        ...
        if (activePlayer) {
          if (this.characters.map((char) => char.position).includes(index)) {
            let enemys = GameRule.getEnemyCharacters(this.characters);
            if (enemys.map((char) => char.position).includes(index)) {
              if (GameRule.attackRadius(activePlayer, this.gamePlay.boardSize).includes(index)) {
  ...
  renderTeam(positions, element, indexesList) {
      let index = Pose.generateRandom(positions);
      if (indexesList.includes(positions[index])) {
        index = Pose.generateRandom(positions);
      }
  ...
  runEnemyTurn() {
    let activeEnemy = GameRule.setActiveEnemy(this.characters, this.gamePlay.boardSize);
    this.moveOrAttack(activeEnemy);
    this.setNextPlayer();
  }

  moveOrAttack() {
    let activeEnemy = GameRule.getCharacterByIndex(Number(localStorage.activeEnemy), this.characters);
    let enemyAttack = GameRule.checkEnemyAttack(activeEnemy, GameRule.getPlayerCharacters(this.characters), this.gamePlay.boardSize);
    let target = GameRule.getCharacterByIndex(enemyAttack[0], this.characters);
    if (enemyAttack.length) {
      if (enemyAttack.length !== 1) {
        target = this.characters.filter((char) => enemyAttack.includes(char.position));
        target = target.sort((a, b) => a.character.health - b.character.health)[0];
      }
      this.makeDamage(activeEnemy,target);
    } else {
      activeEnemy.position = GameRule.checkEnemyMoves(activeEnemy, this.gamePlay.boardSize, this.characters);
      this.gamePlay.redrawPositions(this.characters)
    }
  }
  ...
  healthListener() {
    ...
    if (died.length) {
      ...
      } else if (this.characters[died[0]].position === Number(localStorage.activeEnemy)) {
        localStorage.removeItem('activeEnemy')
      }
      ...
      if (!GameRule.getPlayerCharacters(this.characters).length) {
        this.gameOver();
        return;
      } else if (!GameRule.getEnemyCharacters(this.characters).length) {
        this.levelUp();
      }
    }
  }
